...
(function () {
  ...
  $(".btnKPI8").click(function (e) {
		e.preventDefault();
		let tipo = $(this).attr("data");
		let text = $(this).text();
		$("#textKp8").text(text);
		$.ajax({
			type: "get",
			url: base_url('kpi8/'+tipo),
			success: function (answer) {
				let response = JSON.parse(answer);
				if(response.message.type == "success"){
					$("input[name=seguimiento]").val(response.data)
					$("#incomeChart").html('');
					incomeChartFunc();
				}
			}
  ...
  function totalMotiveAlarm(){
	  let total = JSON.parse($("input[name=totalMotiveAlarm]").val());
	  const motive = []
	  const series = [];
	  total.forEach(row => {
		series.push(row.cantidad_registros);
  ...
  const chartOrderStatistics = document.querySelector('#orderStatisticsChart'),
    orderChartConfig = {
      ...
      series: [$('input[name=alarmsActive]').val()*1, $('input[name=alarmsSuspend]').val()*1],
  ...
  function incomeChartFunc(){
	  ...
	  if (typeof incomeChartEl !== undefined && incomeChartEl !== null) {
		 let incomeChart = new ApexCharts(incomeChartEl, incomeChartConfig);
		 incomeChart.render();
	  }
  }
