...
$("#btnIr").click(function (e) {
	...
	if($idC != ""){
		window.location.href = $siteUrl+$submenu+"/sectors/"+$idC;
	}else{
		toast('bg-danger', "Redireccionamiento", "Verifica que todo este bien seleccionado", 1);
	}
});
...
function barrios(parishes) {
	let url = $("input[name=url]").val()+"districs-search/"+parishes
	$.ajax({
		type: "GET",
		url: url,
		success: function (answer) {
			let response = JSON.parse(answer)
			$("#barrio option:not(:first)").remove();
			if(response.message.type === 'success') {
				toast(
					"bg-success",
					response.message.title,
