...
function initMap() {
	...
	function getDraw(type, drawMap) {
		let id = $("input[name=id").val();
		let url = $("input[name=url]").val() + "drawSector/" + id;
		$.ajax({
			type: "GET",
			url: url,
			success: function (answer) {
	...
	function addMarker(icon){
		...
		google.maps.event.addListener(drawMap, "click", function () {
			toast(
				"bg-warning",
				"Dibujar Sector",
				"No se puede dibujar un sector fuera de su parroquia",
				1
			);
		});
	...
	function viewAll(type = "edit") {
		...
		let url =
			$("input[name=url]").val() +
			"reports/sectors/all-sectors-barrio/" +
			idI;
				...
				if (response.message.type == "success") {
					...
					sectors.forEach((row) => {
						if (row.id != id && type != "view") {
							getDrawOtherSectors(row.id, row.color, row.name, drawMap);
						} else if (type == "view") {
