<h4 class="fw-bold py-3 mb-4">Historial
	<?php foreach ($buttonTop as $row): ?>
		<?php if ($row->id == 'BP0010'): ?>
			<a class="btn btn-info text-white" target="_blank" href="<?= site_url('pdf-history') ?>">
				<?= $row->name ?>
			</a>
		<?php endif; ?>
		<?php if ($row->id == 'BP0011'): ?>
			<a class="btn btn-danger text-white" href="<?= site_url('excel-history') ?>">
				<?= $row->name ?>
			</a>
		<?php endif; ?>
	<?php endforeach; ?>
...
<div class="card">
	<h5 class="card-header">Historial de actividad en el sistema</h5>
